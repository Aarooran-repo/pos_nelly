...
class Pos extends CI_Controller
{
    ...
    public function printkot($number)
    {
        date_default_timezone_set($this->setting->timezone);
        ...
        $ticket = '<h4 class="text-center"><b>KOT OF HOTEL NELLY </b></h4> <div style="clear:both;"></div><span class="float-left"><b>DATE &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp  :&nbsp&nbsp&nbsp  ' . $date . '</b></span><br><div style="clear:both;"><span  class="float-left"><b>TABLE #    &nbsp&nbsp: &nbsp&nbsp ' . sprintf("%02d", $number). '</b></span><div style="clear:both;"><table class="table" cellspacing="0" border="0"><thead><tr><th><em><b>#</b></em></th><th><b>PRODUCT</b></th><th><b>QUANTITY</b></th></tr></thead><tbody>';
        $i = 1;
        $subTotal=0.0;
        $grandTotal=0.0;
        $serviceCharge=0.0;
        $items=0;
        foreach ($posales as $posale) {
            $ticket .= '<tr><td style="text-align:center; width:30px;"><b>' . $i . '</b></td><td style="text-align:left; width:180px;"><b>' . $posale->name . '</b></td><td style="text-align:center; width:50px;"><b>' . $posale->qt . '</b></td></tr>';
            $i ++;
            $subTotal=$subTotal+($posale->qt * $posale->price);
            $items=$items+$posale->qt;
        }
        $serviceCharge=($subTotal*10)/100;
        $grandTotal=$subTotal+$serviceCharge;
        $ticket .= '</tbody></table>';

        $ticket .= '<div style="border-top:1px solid #000; padding-top:10px;"><span class="float-left"><b>Add #: 84,2nd Cross Street <br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Vavuniya</b></span><span class="float-right"><b>Tel #: 0242224477</b></span><p class="text-center" style="margin:0 auto;margin-top:10px;"><b></b></p><br><div class="text-center" style="background-color:#000;padding:5px;width:85%;color:#fff;margin:0 auto;border-radius:3px;margin-top:20px;"><b>' . $this->setting->receiptfooter . '</b></div></div>';
    ...
    public function AddNewSale($type)
    {
        ...
        $saleup = Sale::find('first', array('conditions' => array('id = ?', $sale->id)));
        $saleup->client_id=$number;
        $saleup->clientname='Table '.$number;
        $saleup->save();
        date_default_timezone_set($this->setting->timezone);
        $date1 = date("Y/M/d H:i:s");

        $ticket = '<h4 class="text-center"><b>HOTEL NELLY PVT LTD </b></h4> <div style="clear:both;"></div><span class="float-left"><b>REG # &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp  :&nbsp&nbsp&nbsp Reg 72670</b></span><br><span class="float-left"><b>DATE &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp  :&nbsp&nbsp&nbsp  ' . $date1 . '</b></span><br><div style="clear:both;"><span  class="float-left"><b>TABLE #    &nbsp&nbsp: &nbsp&nbsp ' . sprintf("%02d",$number). '</b></span><div style="clear:both;"><table class="table" cellspacing="0" border="0"><thead><tr><th><em><b>#</b></em></th><th><b>PRODUCT</b></th><th><b>QUANTITY</b></th><th><b>SUBTOTAL</b></th></tr></thead><tbody>';



        $i = 1;
        foreach ($posales as $posale) {
            $ticket .= '<tr><td style="text-align:center; width:30px;"><b>' . $i . '</b></td><td style="text-align:left; width:180px;"><b>' . $posale->name . '</b></td><td style="text-align:center; width:50px;"><b>' . $posale->qt . '</b></td><td style="text-align:right; width:70px; "><b>' . number_format((float)($posale->qt * $posale->price), $this->setting->decimals, '.', '') . '</b></td></tr>';
            $i ++;
        }
        ...
        $height = 20;
        $width = 3;
        $ticket .= '</tbody></table><table class="table" cellspacing="0" border="0" style="margin-bottom:8px;"><tbody><tr><td style="text-align:left;"><b>TOTAL ITEMS</b></td><td style="text-align:right; padding-right:1.5%;"><b>' . $sale->totalitems . '</b></td><td style="text-align:left; padding-left:1.5%;"><b>' . label("Items") . '</b></td><td style="text-align:right;font-weight:bold;"><b>' . $sale->subtotal . '</b></td></tr>';
        if (intval($sale->tax))
$taxAmount=($sale->subtotal*$sale->tax)/100;
            $ticket .= '<tr><td style="text-align:left;"><b>SERVICE CHARGE</b></td><td style="text-align:right; padding-right:1.5%;"></td>
<td style="text-align:left; padding-left:1.5%;"><b>' . $sale->tax . '</b></td>
<td style="text-align:right;font-weight:bold;"><b>' .number_format((float)$taxAmount, $this->setting->decimals, '.', ''). '</b></td>
</tr>';
        $ticket .= '<tr><td colspan="2" style="text-align:left; font-weight:bold; padding-top:5px;"><b>GRAND TOTAL</b></td><td colspan="2" style="border-top:1px dashed #000; padding-top:5px; text-align:right; font-weight:bold;"><b>' . number_format((float)$sale->total, $this->setting->decimals, '.', '') . '</b></td></tr><tr>';
        ...
        switch ($PayMethode[0]) {
            ...
            case '2': // case ckeck
                ...
                break;
            default:
                $ticket .= '<td colspan="2" style="text-align:left; font-weight:bold; padding-top:5px;"><b>PAID</b></td><td colspan="2" style="padding-top:5px; text-align:right; font-weight:bold;"><b>' . number_format((float)$sale->paid, $this->setting->decimals, '.', '') . '</b></td></tr><tr><td colspan="2" style="text-align:left; font-weight:bold; padding-top:5px;"><b>CHANGE</b></td><td colspan="2" style="padding-top:5px; text-align:right; font-weight:bold;"><b>' . number_format((float)(floatval($sale->paid) - floatval($sale->total)), $this->setting->decimals, '.', '') . '</b></td></tr></tbody></table>';
        }
    ...
    public function holdList($registerid)
    {
        ...
        $Tholds = '';
        if (empty($holds))
            $Tholds = '<span style="background-color: #5a2799;color:#ffffff;font-weight: bold" class="Hold selectedHold">1<span style="color:#ffffff" id="Time">TABLE</span></span>';
        else {
            if (empty($posale)) {
                ...
                foreach ($holds as $hold) {
                    if (++ $i === $numItems)
                        $Tholds .= '<span class="Hold selectedHold" style="background-color: #5a2799;color:#ffffff;font-weight:bold" id="' . $hold->number . '"  onclick="SelectHold(' . $hold->number . ')">' . $hold->number . '<span style="color:#ffffff" id="Time">' . $hold->time . '</span></span>';
                    else
                        $Tholds .= '<span class="Hold" style="background-color: #5a2799;color:#ffffff;font-weight:bold" id="' . $hold->number . '"  onclick="SelectHold(' . $hold->number . ')">' . $hold->number . '<span style="color:#ffffff" id="Time">' . $hold->time . '</span></span>';
                }
            } else {
                foreach ($holds as $hold) {
                    if ($hold->number == $posale->number)
                        ...
                        $selected = '';
                    $Tholds .= '<span style="background-color: #5a2799;color:#ffffff;font-weight:bold" class="Hold ' . $selected . '" id="' . $hold->number . '" onclick="SelectHold(' . $hold->number . ')">' . $hold->number . '<span style="color:#ffffff" id="Time">' . $hold->time . '</span></span>';
                }
            }
        }
        echo $Tholds;
