...
<button type="button" onclick="cancelPOS()" class="btn btn-red col-md-6 flat-box-btn"><h5 class="text-bold"><?=label('CANCEL');?></h5></button>
               <button id="C5" type="button" class="btn btn-green col-md-6 flat-box-btn" data-toggle="modal" data-target="#AddSale"><h5 class="text-bold"><?=label('PAYEMENT');?></h5></button>
</div>
<div class="container-fluid">
   <div class="row">
      <ul class="cbp-vimenu" style="display:none">
...
<br>
           <div class="col-xs-12 client-add" style="height:50px">
         
               <button style="float: right;background-color: purple" class="btn btn-add btn-md" onclick="kotBillPrint()"><b>PRINT KOT</b></button>
...
<div  style="visibility: hidden">
            <button id="C6" style="visibility: hidden" type="button" class="btn btn-green col-md-6 flat-box-btn" data-toggle="modal" data-target="#AddSale"><h5 class="text-bold"><?=label('');?></h5></button>
...
<br><br><br>
               ...
               <div class="table-responsive col-sm-12 totalTab">
                   <table class="table">
                       ...
                       <tr>
                           <td class="active"><?=label("Service Charge");?></td>
               ...
               <button type="button" onclick="cancelPOS()" class="btn btn-red col-md-6 flat-box-btn"><h5 class="text-bold"><?=label('CANCEL');?></h5></button>
               <button id="C4" type="button" class="btn btn-green col-md-6 flat-box-btn" data-toggle="modal" data-target="#AddSale"><h5 class="text-bold"><?=label('PAYEMENT');?></h5></button>
...
<!--end first-->
      ...
      <div class="col-md-7 left-side nopadding">
         <div class="row row-horizon">
                  <span style="background-color:red;color:white" class="categories selectedGat"><i class="fa fa-home"></i></span>
                  <?php $count=0;?>
                  <?php foreach ($categories as $category):?>
                     <span style="background-color:purple;color:white" class="categories" id="<?=$category->name;?>"><b><?=$category->name;?></b></span>
                      <?php
             if($count==6||$count==13||$count==21){?>
                <br>
             <?php } ?>
             <?php $count=$count+1;?>
                  <?php endforeach;?>
               ...
               <div class="col-sm-12">
                  <div id="searchContaner">
                      <div class="input-group stylish-input-group">
                          <input type="text" autofocus id="searchProd"  class="form-control"   placeholder="<?=label('Search');?>" >
...
function kotBillPrint(){
    var number = $('.selectedHold').clone().children().remove().end().text();

    $.ajax({
        url : "<?php echo site_url('pos/printKot')?>/"+number,
        type: "POST",
        success: function(data)
        {
            $('#printSection').html(data);
...
$(document).ready(function(){
  $('#ticket').on('shown.bs.modal', function (event) {
 ...
 <div class="modal-dialog" role="document" id="ticketModal">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button id="print_button" type="button" class="btn btn-add hiddenpr" onclick="PrintTicket()"><?=label("print");?></button>
      </div>
    </div>
 </div>
</div>
