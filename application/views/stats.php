<div class="container">
<br>
<a style="margin-bottom: 15px" class="btn btn-add btn-lg" href="<?=base_url();?>"><i class="fa fa-home"></i><?=label(" Home");?></a>
   ...
   <div class="row rangeStat" style="margin-top:50px;">
       <h3 class="col-sm-12"><?=label('Sales Report');?></h3>
       <div class="col-md-6">
           <div class="form-group">
               ...
               <div class="input-group margin-bottom-sm">
                   ...
                   <input class="form-control" id="CustomerRange8" type="text" name="daterange" />
               </div>
           </div>
       </div>
       <div class="col-md-6">
           <button class="cancelBtn btn btn-picker" type="button" onclick="getSalesReport()"><?=label('GetReport');?></button>
       </div>
   </div>

   <div class="row rangeStat" style="margin-top:50px;">
       <h3 class="col-sm-12"><?=label('Sold Inventory Report');?></h3>
       <div class="col-md-6">
           <div class="form-group">
               <label><?=label('SelectRange');?></label>
               <div class="input-group margin-bottom-sm">
                   <span class="input-group-addon RangePicker"><i class="fa fa-calendar fa-fw" aria-hidden="true"></i></span>
                   <input class="form-control" id="CustomerRange9" type="text" name="daterange" />
               </div>
           </div>
       </div>
       <div class="col-md-6">
           <button class="cancelBtn btn btn-picker" type="button" onclick="getInventoryReport()"><?=label('GetReport');?></button>
       </div>
   </div>

    <div class="row rangeStat" style="margin-top:50px;">
       <h3 class="col-sm-12"><?=label('Added Inventory Report');?></h3>
       <div class="col-md-6">
           <div class="form-group">
               <label><?=label('SelectRange');?></label>
               <div class="input-group margin-bottom-sm">
                   <span class="input-group-addon RangePicker"><i class="fa fa-calendar fa-fw" aria-hidden="true"></i></span>
                   <input class="form-control" id="CustomerRange10" type="text" name="daterange" />
               </div>
           </div>
       </div>
       <div class="col-md-6">
           <button class="cancelBtn btn btn-picker" type="button" onclick="getAddedInventoryReport()"><?=label('GetReport');?></button>
       </div>
   </div>
   
   <div class="row rangeStat" style="margin-top:50px;">
       <h3 class="col-sm-12"><?=label('Current Inventory Report');?></h3>
       
       <div class="col-md-4">
           <button class="cancelBtn btn btn-picker" type="button" onclick="getCurrentInventoryReport()"><?=label('GetReport');?></button>
       </div>
       <div class="col-md-4">
...
<script>
    ...
    $(function() {
        ...
        $('input[name="daterangeR"]').daterangepicker();
        var d = new Date().getFullYear();
        $('#CustomerRange8').val('01/01/'+d+' - 12/31/'+d);
        $('#CustomerRange9').val('01/01/'+d+' - 12/31/'+d);
        $('#CustomerRange10').val('01/01/'+d+' - 12/31/'+d);

    });

    function getSalesReport()
    {
        var Range = $('#CustomerRange8').val();
        var start = Range.slice(6,10)+'-'+Range.slice(0,2)+'-'+Range.slice(3,5);
        var end = Range.slice(19,23)+'-'+Range.slice(13,15)+'-'+Range.slice(16,18);

        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('reports/getSalesReport')?>/",
            type: "POST",
    ...
    function getInventoryReport()
    {
      ...
      //  console.log(end);
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('reports/getInventoryReport')?>/",
            type: "POST",
            data: { start: start, end: end},
            success: function(data)
            {
    ...
    function getAddedInventoryReport()
    {
        var Range = $('#CustomerRange10').val();
        var start = Range.slice(6,10)+'-'+Range.slice(0,2)+'-'+Range.slice(3,5);
        var end = Range.slice(19,23)+'-'+Range.slice(13,15)+'-'+Range.slice(16,18);
      //  console.log(start);
      //  console.log(end);
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('reports/getAddedInventoryReport')?>/",
            type: "POST",
            data: { start: start, end: end},
            success: function(data)
            {
    ...
    function getCurrentInventoryReport()
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('reports/getCurrentInventoryReport')?>/",
            type: "POST",
            data: "",
            success: function(data)
            {
